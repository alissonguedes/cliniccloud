			...
			<div class="col s12 m6 l3 grid-view animated fadeIn slow delay-{{ $i }}">
				<div class="pacientes card card-border border-radius-6 z-depth-3 gradient-45deg-indigo-light-blue" @if ($paciente->obito === '1') style="filter: grayscale(1)" @endif>
					<div class="card-content white-text">
						...
						{? $link = ($paciente->status === '1' && $paciente->obito === '0') ? 'data-link='. route('clinica.pacientes.{id}.agendamento',$paciente->id) : null; ?}
						...
						<div class="acoes flex flex-center mt-5">
							<button data-href="{{ go('clinica.pacientes.{id}.prontuarios', $paciente->id) }}" name="id" id="{{ $paciente->id }}" data-target="prontuario" class="waves-effect gradient-45deg-deep-orange-orange center-align icon-background circle white-text z-depth-3 mx-auto border-0 " data-tooltip="Prontuário">
